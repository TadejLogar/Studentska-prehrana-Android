...
public class RestaurantModel extends Model {
    ...
    public void addRestaurantsFast(Vector<com.sciget.studentmeals.client.service.data.RestaurantData> list) {
        ...
        final int mapImageSha1 = ih.getColumnIndex("mapImageSha1");
        ...
        for (com.sciget.studentmeals.client.service.data.RestaurantData data : list) {
            ...
            RestaurantData restaurant = new RestaurantData(data.id, data.hash, data.name, data.address, data.post, data.country, data.price, data.phone, data.openWorkdayFrom, data.openWorkdayTo, data.openSaturdayFrom, data.openSaturdayTo, data.openSundayFrom, data.openSundayTo, data.locationLatitude, data.locationLongitude, data.features, data.message, data.imageSha1, null);
            ...
            ih.bind(mapImageSha1, restaurant.mapImageSha1);
    ...
    public ArrayList<RestaurantData> getAllRestaurants() {
        ArrayList<RestaurantData> restaurants = new ArrayList<RestaurantData>();
        Cursor cursor = rawQuery("SELECT id, hash, name, address, post, country, price, phone, openWorkdayFrom, openWorkdayTo, openSaturdayFrom, openSaturdayTo, openSundayFrom, openSundayTo, locationLatitude, locationLongitude, features, message, imageSha1, mapImageSha1 FROM " + RestaurantData.NAME);
        while (cursor.moveToNext()) {
            Cursor resultSet = cursor;
            RestaurantData restaurant = new RestaurantData(resultSet.getInt(0), resultSet.getString(1), resultSet.getString(2), resultSet.getString(3), resultSet.getString(4), resultSet.getString(5), resultSet.getDouble(6), resultSet.getString(7), toTime(resultSet.getString(8)), toTime(resultSet.getString(9)), toTime(resultSet.getString(10)), toTime(resultSet.getString(11)), toTime(resultSet.getString(12)), toTime(resultSet.getString(13)), resultSet.getDouble(14), resultSet.getDouble(15), resultSet.getString(16), resultSet.getString(17), resultSet.getString(18), resultSet.getString(19));
    ...
    public void setRestaurantMapImageHash(int restaurantId, String newHash) {
        update("UPDATE " + RestaurantData.NAME + " SET mapImageSha1 = ? WHERE id = ?", new Object[] { newHash, restaurantId });
    }
}
